...
export const stringifyArr = (cartItems: Cart[]): string => {
  return cartItems
    .map((item: Cart) => {
      return `${item.id},${item.imageUrl},${item.title},${item.price},${item.loved},${item.count},${item.description}`;
    })
  ...
  if (cartItems.length > 0) {
    cartItems.forEach((cartItem, i) => {
      ...
      items = [
        ...
        {
          id: cartItemPieces[0],          
          imageUrl: cartItemPieces[1],
          title: cartItemPieces[2],                   
          price: cartItemPieces[3],
          loved: cartItemPieces[4],
          count: Number(cartItemPieces[5]),
          description: cartItemPieces[6], 
        }
      ];
    });
  }
